...
from numba import float64, jit, prange, uint16
from PIL import Image

from . import function
# from .function_fast import function_1d, function_2d
# from .parameter import lattice, psf, system
...
class image():
    ...
    def generate_lattice_sites(self, Nsite=2*10+1, rxlim=10, rylim=10,
                               unit='um', limits='circle', origin='best',
                               ROI_origins=None):
        ...
        x0_all = self.system.lattice['Origins'][label_x0]
        y0_all = self.system.lattice['Origins'][label_y0]
        ...
        if origin == 'best':
            ...
            for x0_tmp, y0_tmp in zip(x0_all, y0_all):
    ...
    def set_ROI(self, ROI=None, unit='um'):
        if ROI is None:
            ...
            ROI_px = {'x0 (px)': [id_x0], 'x1 (px)': [id_x1],
                      'y0 (px)': [id_y0], 'y1 (px)': [id_y1]}
